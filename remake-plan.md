...
## Точечные правки адаптива (по ревью в Chrome)
- [x] Пагинация: «В начало» / «Дальше» больше не скрываются на мобильном; на первой/последней странице — неактивное состояние (`.pagination__nav.is-disabled`, `<span>`, `pointer-events: none`). Обновлён `catalog-mock.js`.
- [x] `catalog-hero__tags` ≤768: 1px outline кнопок обрезался скролл-контейнером → добавлен вертикальный `padding: 4px` в `&__tags` (≤768 и ≤360).
- [x] `header__socials--tablet`: скрыты ниже 1007px (`@include vp(1007)`) — раньше переносились на новую строку. Заодно починен баг миксина `vp()` (число без единиц давало `max-width: 1007` без `px`).
- [x] `.catalog-hero`: резиновые боковые поля `clamp(16px, 3.2vw, 40px)` + `max-width` с запасом 80px — секция больше не прилипает к краям в диапазоне 1025–1440; убраны дублирующие `padding` в `vp-1024`/`vp-756`.
- [x] Промо-карточка (`product-promo-card`) больше не пропадает при сортировке/фильтрации/пагинации: помечена `data-list-pinned`, `catalog-ajax.js` сохраняет такие элементы (узлы не пересоздаются) и возвращает на исходные позиции сетки после каждого AJAX-обновления; из ответа сервера `[data-list-pinned]` вырезаются, чтобы не дублировались. `handleEmpty` считает только товарные карточки.
- [x] Категорийные теги в hero (`catalog-hero__tags`) теперь триггерят AJAX каталога: помечены `data-hero-tags data-filter-group="category"`, у каждого `data-filter-value` (servers|storage|network|ups|software) и реальный `href="?category=..."` (работает и без JS). `filters.js` слушает их отдельно — одиночный выбор, повторный клик снимает, состояние восстанавливается из URL, комбинируется с фильтрами сайдбара. `catalog-mock.js`: у товаров добавлено поле `category`, параметр в контракте.
